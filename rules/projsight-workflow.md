...
| 緩和タスク完了後 | `upsert_risk(status: 'mitigated')` or `upsert_risk(status: 'closed')`      |
| リスク顕在化時   | `upsert_risk(status: 'occurred')` → `upsert_issue` → リンク                |
...
| 修正完了後       | `upsert_issue(status: 'resolved')`                                        |
