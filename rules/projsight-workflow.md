...
- 開始 → `start_work(taskId)` — コンテキスト自動返却、get_project_context 不要
...
納品物（deliverable）のステータスは **エンジニアが明示的に管理** する。自動遷移はない。
...
- DRが必要な場合 → `add_dr` を実装ステップに含める（ローカルファイル作成は任意、プロジェクトの CLAUDE.md に従う）
- DR品質基準（`0000-template.md` チェックリスト準拠）:
...
- DR ローカルファイル: 任意（非推奨）。ProjSight 登録が正。設計ドキュメント: ローカル不要（MCP が正）
...
- ローカルファイル: MCP が正（Single Source of Truth）。DR ローカルファイルは任意（非推奨）。イシュー・タスクは MCP 登録後に削除
