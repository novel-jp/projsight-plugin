...
1. `docs/dr/NNNN-<slug>.md` をテンプレート準拠で作成
2. `add_dr(projectId, title, context, decision, alternatives, consequences, ...)` を実行
   - #5 の場合は `supersedes` パラメータを指定
3. 返却された DR の `id` と `number` を記録
...
2. ローカルファイルは不要（MCP が正）
3. 返却された設計ドキュメントの `id` と `number` を記録
...
1. `list_deliverables(projectId)` で紐付け先の納品物を確認
2. ユーザーと紐付け先を確認（不明な場合）
3. `create_task(projectId, title, deliverableId, description)` を実行
   - **description は Markdown 形式**で、以下の構成で記述すること:
     - `## 背景` — なぜこのタスクが必要か
     - `## 対応内容` — 何をするか（箇条書き）
     - `## 完了条件` — チェックリスト形式
   - 基準: サイドペインで読んで作業着手できる程度の情報量
4. 返却された Task の `id` を記録
...
2. 返却された `relatedDeliverable.status` が `draft` の場合 → `update_deliverable(deliverableId, status: 'in_progress')` を実行して納品物を着手状態にする。
...
- [ ] 紐づく納品物が draft なら in_progress に遷移済み
