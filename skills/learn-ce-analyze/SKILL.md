...
> **タスクについて**: この演習のタスクは `/learn` ハブから起動された際に自動作成されています。演習の最後（Step 4）で学習記録を ProjSight に保存し、タスクを完了します。
...
2. **まず CLAUDE.md の見出し一覧（セクション構成）を提示し、全体像を把握してもらう**
   ...
   - > **補足**: CLAUDE.md 内に登場する DR-xxxx や DD-xxxx という番号は、ProjSight に記録された意思決定記録（DR）や設計ドキュメント（DD）の参照先です。この演習では番号の中身を確認する必要はありません。
...
- **馴染みがある場合** → 足場かけの比喩部分は省略し、直接 3 つの問いに進む
- **馴染みがない場合** → 以下の誘導文をフルで提示してから問いに進む:
...
#### B) まとめて進める場合（ファストトラック）

- セクション 4〜6 を一度に提示する
- 2 つの問いを 3 セクション分まとめて回答してもらう（セクションごとに分けて回答しても、横断的にまとめて回答しても OK）
- 回答後、claude-md-analysis.json の解説を 3 セクション分まとめて見せる

#### セクション 6（デプロイ手順）の解説時の補足
