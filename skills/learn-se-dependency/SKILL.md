...
各タスクの **タイトル** と **id（UUID）** の対応表を提示する。

```
例:
| # | タスク名           | id (UUID)                            |
|---|--------------------|--------------------------------------|
| 1 | DB スキーマ設計     | a1b2c3d4-...                         |
| 2 | API 実装           | e5f6g7h8-...                         |
| 3 | モデル定義         | i9j0k1l2-...                         |
| 4 | UI 実装            | m3n4o5p6-...                         |
```

**タスクが 4 件未満の場合**: 依存関係の演習には最低 4 件のタスクが必要です。不足している場合は「演習用に追加タスクを 2〜3 件作成しましょう」と案内し、`upsert_task` で作成してから次へ進む。
...
```
「この図から読み取れること:
- DB スキーマ設計と認証基盤は並列に着手できる
- API 実装は 2 つのタスクにブロックされている（ボトルネック候補）
- UI 実装は最後に着手する」
```
...
- **この演習では `blocks` を使います**。他の linkType（resolves, mitigates 等）は後の演習で使う機会があります

```
「blocks リンクの方向に注意してください:
  create_link(
    sourceType='task', sourceId=<タスクAのUUID>,
...
Step 3 で登録した依存関係の **target 側**（ブロックされる側）のタスクを `start_work` で開始してみる。
