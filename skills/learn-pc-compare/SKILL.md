...
```bash
mkdir -p learn-workspace/01-01-compare
```

次に、受講者に指示する:

```
「演習用ディレクトリ learn-workspace/01-01-compare/ を作成しました。

では、曖昧な指示で実装させてみましょう。
このセッション内でそのまま、以下のように指示してください:」
```
...
```
「learn-workspace/01-01-compare/ の中に実装するよう指示してください。
...
全部揃ったら、learn-workspace/01-01-compare/ 内の別ディレクトリ（例: structured/）に
実装するよう Claude Code に指示してください。」
```

> 受講者がスムーズに 4 要素を書ける場合（ベテラン等）は、段階的ガイドをスキップして一括で書いてもらって構いません。
...
- 演習の実装は `learn-workspace/01-01-compare/` に格納する。本番プロジェクトのコードと混在させない
